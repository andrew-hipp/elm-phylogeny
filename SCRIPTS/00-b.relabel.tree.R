...
if(doNiceFig) tr.ulm <- drop.tip(tr.ulm, og)

source('../SCRIPTS/99.relabelTree.R')

tr.out <- relabel.tree()
tr.ulm.outFile <- paste(tr.ulm.outFile, format(Sys.time(), "_%Y-%m-%d_%H%M%S.pdf"), sep = '')
...
nodelabels(node = seq(from = length(tr.out$tip.label) + 1,
          ...
          frame = 'n')
if(doNiceFig) {
  dat.singletons <- read.csv('SUPPLEMENT.tipsInSingletonsTree.csv', as.is = T, row.names = 1)
